...
bool ParticleShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*)L"../Engine/ParticleSystem/particle.vs", (WCHAR*)L"../Engine/ParticleSystem/particle.ps");
	if(!result)
		return false;
...
bool ParticleShaderClass::Render(float DeltaSeconds, ID3D11DeviceContext* deviceContext, int indexCount, Matrix worldMatrix, Matrix viewMatrix, 
								Matrix projectionMatrix, Vector3 cameraPosition, ID3D11ShaderResourceView* texture)
{
	bool result;

	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(DeltaSeconds, deviceContext, worldMatrix, viewMatrix, projectionMatrix, cameraPosition, texture);
	if(!result)
		return false;
...
bool ParticleShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile((WCHAR*)L"../Engine/ParticleSystem/ParticleCompute.hlsl", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", "cs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			OutputShaderErrorMessage(errorMessage, hwnd, (WCHAR*)L"../Engine/ParticleSystem/ParticleCompute.hlsl");
		}
		// If there was  nothing in the error message then it simply could not find the file itself.
		else
		{
			MessageBox(hwnd, (WCHAR*)L"../Engine/ParticleSystem/ParticleCompute.hlsl", L"Missing Shader File", MB_OK);
		}
	...
	polygonLayout[2].SemanticName = "CORNER";
	...
	polygonLayout[3].SemanticName = "ANGLE";
	polygonLayout[3].SemanticIndex = 0;
	polygonLayout[3].Format = DXGI_FORMAT::DXGI_FORMAT_R32G32_FLOAT;;
	...
	// Setup the description of the dynamic camera constant buffer that is in the compute shader.
	matrixBufferDesc.ByteWidth = sizeof(CommonDataForParticleSystem);

	// Create the constant buffer pointer so we can access the compute shader constant buffer from within this class.
	result = device->CreateBuffer(&matrixBufferDesc, NULL, &m_commonBuffer);
	if (FAILED(result))
		return false;
...
bool ParticleShaderClass::SetShaderParameters(float DeltaSeconds, ID3D11DeviceContext* deviceContext, Matrix worldMatrix, Matrix viewMatrix, 
											 Matrix projectionMatrix, Vector3 cameraPosition, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
    D3D11_MAPPED_SUBRESOURCE mappedResource, mappedResource2;
	MatrixBufferType* MatrixDataPtr;
	CommonDataForParticleSystem* CommonDataPtr;
	...
	// Lock the constant buffer so it can be written to.
	result = deviceContext->Map(m_commonBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource2);
	if (FAILED(result))
		return false;

	// Get a pointer to the data in the constant buffer.
	CommonDataPtr = (CommonDataForParticleSystem*)mappedResource2.pData;

	CommonDataPtr->cameraPosition = cameraPosition;
	CommonDataPtr->DeltaSeconds = DeltaSeconds;

	// Unlock the constant buffer.
	deviceContext->Unmap(m_commonBuffer, 0);

	deviceContext->CSSetConstantBuffers(0, 1, &m_commonBuffer);
