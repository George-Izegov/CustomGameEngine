...
public:
    ...
    void setVector(std::string key, DirectX::SimpleMath::Vector3 value) { vectors[key] = value; }
    DirectX::SimpleMath::Vector3 getVector(std::string key)
    {
        if (vectors.find(key) == vectors.end()) {
            vectors[key] = DirectX::SimpleMath::Vector3(0,0,0);
        }
        return vectors[key];
    }
...
protected:
    ...
    std::unordered_map<std::string, DirectX::SimpleMath::Vector3> vectors;
...
typedef std::shared_ptr<BrainTree::Blackboard> SPBlackboard;
typedef std::shared_ptr<BrainTree::BehaviorTree> SPBehaviorTree;
typedef std::shared_ptr<BrainTree::Node> SPNode;