...
// DirectX data
static bool                     g_pVsync_enabled;
static int                      g_pVideoCardMemory;
static char                     g_pVideoCardDescription[128];
...
static ImVec4                   g_pColor = ImVec4(0.f, 0.f, 0.f, 1.00f);
...
static void ImGui_ImplDX11_SetupRenderState(ImDrawData* draw_data, ID3D11DeviceContext* ctx, int screenWidth, int screenHeight)
{
    // Setup viewport
    D3D11_VIEWPORT vp;
    ...
    /*ctx->OMSetRenderTargets(1, &g_pRenderTargetView, g_pDepthStencilView);
    ctx->ClearRenderTargetView(g_pRenderTargetView, (float*)&g_pColor);*/
}

// Render function
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data, int screenWidth, int screenHeight, float screenDepth, float screenNear)
{
    ...
    ImGui_ImplDX11_SetupRenderState(draw_data, ctx, screenWidth, screenHeight);
...
bool    ImGui_ImplDX11_CreateDeviceObjects(int screenWidth, int screenHeight)
{
    if (!g_pd3dDevice)
        return false;
    if (g_pFontSampler)
    ...
    // Create the blending setup
    {
        ...
        result = g_pd3dDevice->CreateBlendState(&blendDesc, &g_pBlendState);
        if (FAILED(result))
        {
            return false;
        }
    ...
    // Create the rasterizer state
    {
        ...
        result = g_pd3dDevice->CreateRasterizerState(&rasterDesc, &g_pRasterizerState);
        if (FAILED(result))
        {
            return false;
        }
    ...
    // Create depth-buffer desc
    {
        ...
        result = g_pd3dDevice->CreateTexture2D(&depthBufferDesc, NULL, &g_pDepthStencilBuffer);
        if (FAILED(result))
        {
            return false;
        }
    ...
    // Create depth-stencil State
    {
        ...
        depthStencilDesc.FrontFace.StencilFailOp = depthStencilDesc.FrontFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
        ...
        result = g_pd3dDevice->CreateDepthStencilState(&depthStencilDesc, &g_pDepthStencilState);
        if (FAILED(result))
        {
            return false;
        }
...
void ImGui_ImplDX11_NewFrame(int screenWidth, int screenHeight)
{
    if (!g_pFontSampler)
